...
function renderTonightGrid(){{
  ...
  TONIGHT_GAMES.forEach(gm=>{{
    ...
    const hWin=Math.round((hp/tot)*100+3),aWin=100-hWin;
    const fav=hWin>=50?gm.home:gm.away,favPct=Math.max(hWin,aWin);
    const ou=((H.ppg||112)+(A.ppg||112))*0.97;
    const linesHtml=!isFinal?`<div style="margin-top:8px;display:flex;gap:8px;font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:12px;letter-spacing:1px"><span style="color:#f0f4f8">FAV: ${{fav.split(' ').pop().toUpperCase()}} ${{favPct}}%</span><span style="color:var(--gray)">·</span><span style="color:var(--gold)">O/U ${{ou.toFixed(1)}}</span></div>`:'';
